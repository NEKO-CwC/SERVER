...
systemctl is-enabled bypass.service sing-box.service
...
ip -4 rule show | grep -Eq '^4004:[[:space:]]+from all to 100\.64\.0\.0/10 lookup 52$'
ip -6 rule show | grep -Eq '^4004:[[:space:]]+from all to fd7a:115c:a1e0::/48 lookup 52$'
...
echo "[verify] checking cleanup and restart"
systemctl stop sing-box.service
systemctl stop bypass.service
...
systemctl start bypass.service
